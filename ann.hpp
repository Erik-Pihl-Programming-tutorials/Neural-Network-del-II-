...
private:
   ...
   ********************************************************************************/
   void check_training_data_size(void)
   {
      ...
      {
         const auto num_sets = this->train_in_.size() <= this->train_out_.size() ?
            this->train_in_.size() : this->train_out_.size();
         this->train_in_.resize(num_sets);
         this->train_out_.resize(num_sets);
      }
      return;
   }
   ...
   ********************************************************************************/
   void randomize_training_order(void)
   {
      for (std::size_t i = 0; i < this->num_training_sets(); ++i)
      {
         ...
         const auto copy = this->train_order_[i];
         this->train_order_[i] = this->train_order_[r];
         this->train_order_[r] = copy;
      }
      return;
   }
